class Api::V1::InvitationsController < ApplicationController
  ...
  def show
    ...
    uid = params[:id]
    latest_invitation = Invitation.where("inviter = :uid OR invitee = :uid", uid:)
    ...
    if latest_invitation.nil?
      ...
      # テーブルに保存
      invitation = Invitation.new(inviter: uid, invitee: nil, invite_digest:, status: "pending")
      if invitation.save
        # URLを生成
  ...
  def create
    ...
    elsif invitation.expires_at < Time.zone.now
